...
- [x] `infra/cdk/src/foundation.ts` and tests -- schedule the isolated worker every minute with reserved concurrency one, secret read, scoped Dynamo actions, DLQ/error/staleness/quarantine alarms, and no serving dependency; completed streams return without paid calls.
...
- 2026-08-15: A documentation-led live audit found the remaining acquisition blocker. SharpAPI documents `/events` with `limit <= 200`, `offset <= 5000`, and comma-separated sport/league filters; the staging account currently exposes more than 18,000 events, so the unfiltered offset walk cannot complete. The include-empty `/sports` and `/leagues` catalogs remain the dynamic source of truth and no sport or league allowlist is introduced. The event sweep freezes deterministic catalog-driven partitions; a later live correction below tightened exact-league use to fully reconciled sport membership and denominators. `/odds` keeps the documented opaque cursor flow with identical filters. A league slug that cannot be represented in the documented comma-separated grammar is quarantined as provider inconsistency and forces fail-closed sport-wide acquisition rather than partial publication. Stable non-2xx `error.code` values, bounded provider timestamps, and catalog/filter inconsistencies remain evidence rather than prose-matched or silently discarded data.
...
- 2026-08-15: The first post-deploy staging run proved that broad sport-only filters are not a reliable default even for current nonempty catalog sports: `horse_racing`, `olympics`, and `unknown` returned bounded `400 invalid_filter`, while the soccer sport denominator exceeded the documented offset reach. Event planning now uses exact league partitions only when complete league membership and event/live denominators reconcile to the sport catalog. Missing, quarantined, or mismatched league coverage falls back to the sport-wide request and remains unpublished if the provider rejects it; a partial league plan can never appear complete. Exact all-zero catalogs complete without a paid Events request, while any positive live counter remains active. The worker trigger cadence is one minute so incomplete opaque Odds cursors receive frequent continuation opportunities; staging reconciliation still measures actual checkpoint age and rejects a backlogged or expired cursor rather than assuming the schedule is a guarantee. KEEP: no acquisition allowlist, no silent row loss, fail-closed publication, exact account reserve, and dynamic catalog-driven recovery.

## Design Notes

Collection coverage and product support are separate state machines. The landing layer stores source observations in two alternating bounded slots rather than verbatim responses; a conditional checkpoint selects the complete `{slot, sweepId}` logical view while a running sweep leaves the prior completed slot and sweep intact. A changed crash replay or pagination cycle abandons only the inactive sweep and restarts it under a new ID, so partial rows cannot mix with the last good dataset or the replacement sweep. Provider `updated_at` remains bounded evidence rather than a snapshot fence. Event acquisition freezes deterministic catalog-driven partitions, reconciles the sequential `/sports` and `/leagues` membership union by `(sport, league)`, and uses exact league groups only when their complete membership and event/live counters cover the sport denominator. Any missing or inconsistent league coverage uses a sport-wide fail-closed request rather than publishing a partial generation. It walks each partition only within SharpAPI's documented offset ceiling and reconciles the exact provider denominator per partition before advancing. Cursor odds use the provider's opaque continuation with unchanged filters and require exact cursor progression plus a valid terminal page. Values are idempotent within a sweep, removed identities disappear only at the atomic checkpoint boundary, and existing canonical odds history remains immutable. Quarantine is a first-class capture outcome, not permission to drop data.
