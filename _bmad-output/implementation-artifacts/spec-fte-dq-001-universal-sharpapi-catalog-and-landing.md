...
Collection coverage and product support are separate state machines. The landing layer stores source snapshots in two alternating bounded slots rather than verbatim responses; a conditional checkpoint selects the complete `{slot, sweepId}` logical view while a running sweep leaves the prior completed slot and sweep intact. Drift or changed crash replay abandons only the inactive sweep and restarts it under a new ID, so partial rows cannot mix with the last good dataset or the replacement sweep. Snapshot values are idempotent within a sweep, removed identities disappear only at the atomic checkpoint boundary, and existing canonical odds history remains immutable. Quarantine is a first-class capture outcome, not permission to drop data.
